          ...
          {/* Breadcrumbs */}
              <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
                      <nav className="flex items-center gap-2 text-sm text-gray-500" aria-label="Breadcrumb">
                                <Link href="/" className="hover:text-[#8B6914] transition-colors">Hem</Link>Link>
                                <span>/</span>span>
                                <Link href="/deals" className="hover:text-[#8B6914] transition-colors">Deals</Link>Link>
                                <span>/</span>span>
                                <Link href={`/deals?category=${deal.category}`} className="hover:text-[#8B6914] transition-colors capitalize">
                                  {categoryLabel}
                                </Link>Link>
                                <span>/</span>span>
                                <span className="text-gray-800 font-medium truncate max-w-[200px]">{deal.title}</span>span>
                      </nav>nav>
              </div>div>
                        ...
                        {/* Left: Image + Info */}
                          ...
                          <span className="absolute top-4 left-4 bg-[#C9A84C]/20 text-[#C9A84C] text-xs font-bold px-3 py-1.5 rounded-full border border-[#C9A84C]/40 uppercase tracking-widest">
                                            ✦ Utvald Deal
                                                          ...
                                                          <span className="text-8xl select-none">✨</span>span>
                                            </div>div>
                                  ...
                                  {/* Title */}
                                            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4 leading-tight">{deal.title}</h1>h1>
                                
                                  {/* Description */}
                                  {deal.description && (
                        <p className="text-gray-600 text-base leading-relaxed mb-6">{deal.description}</p>p>
                                            )}
                                
                                  {/* What's included */}
                                  {deal.includes && Array.isArray(deal.includes) && deal.includes.length > 0 && (
                        <div className="bg-white rounded-2xl p-6 mb-6 border border-gray-100">
                                        <h2 className="text-base font-bold text-gray-900 mb-4">Vad ingår</h2>h2>
                                        <ul className="space-y-2">
                                          {deal.includes.map((item: string, i: number) => (
                                              <li key={i} className="flex items-center gap-3 text-gray-700">
                                                                    <span className="text-[#2D5A3A] font-bold text-lg">✓</span>span>
                                                                    <span>{item}</span>span>
                                              </li>li>
                                            ))}
                                        </ul>ul>
                          ...
                          <div className="bg-white rounded-2xl p-4 border border-gray-100">
                                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Gäller till</p>p>
                                            <p className="text-gray-900 font-medium">
                                              ...
                                              {spotsRemaining !== null && (
                          <div className="bg-white rounded-2xl p-4 border border-gray-100">
                                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Platser kvar</p>p>
                                            <p className={`font-medium ${spotsRemaining === 0 ? 'text-red-600' : spotsRemaining < 10 ? 'text-orange-600' : 'text-gray-900'}`}>
                                              ...
                                              {deal.rating && (
                          <div className="bg-white rounded-2xl p-4 border border-gray-100">
                                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Betyg</p>p>
                                            <p className="text-gray-900 font-medium">
                          ...
                          <div className="bg-white rounded-2xl p-4 border border-gray-100">
                                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Sålda</p>p>
                                            <p className="text-gray-900 font-medium">🔥 {deal.sold_count} köp</p>p>
                          </div>div>
                                                          )}
                                            </div>div>
                                </div>div>
                                              ...
                                              {/* Price breakdown */}
                                                          <div className="border-t border-gray-100 pt-4 mb-6 space-y-2 text-sm">
                                                                          <div className="flex justify-between text-gray-500">
                                                                                            <span>Ordinarie pris</span>span>
                                                                                            <span>{deal.original_price?.toLocaleString('sv-SE')} kr</span>span>
                                                                          </div>div>
                                                            {discount && discount > 0 && (
                            <div className="flex justify-between text-[#2D5A3A] font-medium">
                                                <span>Valör-rabatt</span>span>
                                                <span>-{discount}%</span>span>
                                                                          ...
                                                                          <div className="flex justify-between font-bold text-gray-900 text-base border-t border-gray-100 pt-2 mt-2">
                                                                                            <span>Totalt</span>span>
                                                                                            <span>{deal.deal_price?.toLocaleString('sv-SE')} kr</span>span>
                                                                          </div>div>
                                                          </div>div>
                                              ...
                                              {/* CTA */}
                                                          ...
                                                          <p className="text-center text-xs text-gray-400 mt-3">
                                                                          Köp = acceptera <Link href="/villkor" className="underline hover:text-gray-600">köpvillkor</Link>Link>
                                                          </p>p>
                                            
                                              {/* Trust signals */}
                                                          <div className="mt-5 space-y-2 text-sm text-gray-600">
                                                                          <div className="flex items-center gap-2">
                                                                                            <span>🔒</span>span> <span>Säker betalning</span>span>
                                                                          </div>div>
                                                                          <div className="flex items-center gap-2">
                                                                                            <span>📧</span>span> <span>Voucher direkt på e-post</span>span>
                                                                          </div>div>
                                                                          <div className="flex items-center gap-2">
                                                                                            <span>✅</span>span> <span>14 dagars ångerrätt</span>span>
                                                                          </div>div>
                                                          </div>div>
                                            </div>div>
                                </div>div>
