  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'login') {
      ...
      if (error) {
        ...
        else if (profile?.role === 'merchant') router.push('/merchant')
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <a href="/" style={{ textDecoration: 'none' }}>
            <div style={{ fontSize: 32, fontFamily: 'Georgia, serif', color: G, letterSpacing: 2 }}>Valör</div>
            <div style={{ fontSize: 11, letterSpacing: 4, color: AU, textTransform: 'uppercase', marginTop: 4 }}>Premium Deals</div>
          </a>
        </div>
        ...
        <div style={{ backgroundColor: WH, borderRadius: 16, padding: 40, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 13, color: G, marginBottom: 6, fontWeight: 500 }}>E-postadress</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                placeholder="din@email.se"
                style={{ width: '100%', padding: '12px 16px', border: `1px solid ${LG}`, borderRadius: 8, fontSize: 14, fontFamily: 'Inter, sans-serif', outline: 'none', boxSizing: 'border-box' }} />
            </div>
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: 'block', fontSize: 13, color: G, marginBottom: 6, fontWeight: 500 }}>Lösenord</label>
          ...
          {mode === 'login' && (
            <div style={{ textAlign: 'center', marginTop: 20 }}>
              <a href="#" style={{ fontSize: 13, color: GR, textDecoration: 'none' }}>Glömt lösenordet?</a>
            </div>
          )}
        </div>
